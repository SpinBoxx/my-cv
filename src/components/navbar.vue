<script setup>
  ...
  window.addEventListener("scroll", () => {
    (window.scrollY !== 0) ? stickyNavbar.value = true : stickyNavbar.value = false;
  })

</script>

<template>
  <nav :class="[stickyNavbar? 'border-gray-200 shadow-md fixed bg-white z-50' : 'absolute',{'z-10 bg-white': isMobile} ]" class="w-screen pl-0 pr-7 py-2.5 rounded sm:px-4 dark:bg-gray-800">
    <div class="container flex flex-wrap justify-between items-center mx-auto">
      <img src="../assets/mimo.png" alt="" class="h-20 w-22">
      <div class="flex md:order-2 z-40">
        <button type="button" class="text-white hidden md:block bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-3xl text-sm px-6 py-2.5 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
          Download my CV
        </button>
      ...
      <div :class="[!isMobile ? 'hidden' : 'z-10']" class="justify-between items-center w-full bg-white rounded p-5 md:bg-transparent md:flex md:w-auto md:order-1" id="mobile-menu-4">
        <ul class="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
          <li>
            <a href="#home" class="block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">
              Home
            </a>
          </li>
          <li>
